create database if not exists P1934;
use P1934;
...
select s.user_id, round(ifnull(ifnull(c2.cnt1, 0) / c1.cnt, 0), 2) confirmation_rate
from Signups s
         left join (select user_id, count(*) cnt from Confirmations group by user_id) c1 on s.user_id = c1.user_id
         left join(select user_id, count(*) cnt1 from Confirmations where action = 'confirmed' group by user_id) c2

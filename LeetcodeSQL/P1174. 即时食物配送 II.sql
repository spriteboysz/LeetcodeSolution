create database if not exists P1174;
use P1174;
...
select round(count(*) * 100 / (select count(distinct customer_id) from Delivery), 2) immediate_percentage
from Delivery
where (customer_id, order_date) in (select customer_id, min(order_date) from Delivery group by customer_id) and

create database if not exists P1661;
use P1661;
...
select machine_id, round(avg(total), 3) processing_time
from (select machine_id, sum(if(activity_type = 'start', -timestamp, timestamp)) total
      from Activity

...
"""
Author: Deean
Date: 2026-06-20 16:59
FileName: P2016. 增量元素之间的最大差值.py
Description:
"""
...
class Solution:
    def maximumDifference(self, nums: List[int]) -> int:
        maximums = [max(nums[i + 1:], default=0) for i, _ in enumerate(nums)]
        return max([maximum - num for num, maximum in zip(nums, maximums) if maximum - num > 0], default=-1)


if __name__ == '__main__':
    solution = Solution().maximumDifference(
